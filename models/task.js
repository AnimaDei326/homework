    ...
    update: function(filtr, callback){
        pool.getConnection(function(err, connection){
            if(err){
                ...
                let {table, setColumn, setValue, setColumn2, setValue2, whereColumn, whereValue} = filtr;
                //console.log(whereValue);
                connection.query("UPDATE ?? SET ?? = ?, ?? = ? WHERE ?? = ?", 
                                [table, setColumn, setValue, setColumn2, setValue2, whereColumn, whereValue], function(err, result){
                    if(err){
                        console.log("Error", err);
                        callback(err);
    ...
    add: function(filtr, callback){
        pool.getConnection(function(err, connection){
            if(err){
                ...
                //console.log("INSERT INTO " + table + " SET " + setColumn + " = " + setValue);
